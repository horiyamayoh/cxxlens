...
/** @file flow.hpp @brief Stable flow contract declarations. */
...
namespace cxxlens::flow
{
	...
	class source_model
	{
	  public:
		...
		[[nodiscard]] static source_model parameter(select::symbol_selector callable,
													std::size_t index);
		...
		[[nodiscard]] source_model metadata(std::string id,
	...
	{
	  public:
		[[nodiscard]] static sink_model call_argument(select::call_selector call,
													  std::size_t index);
		...
		[[nodiscard]] sink_model metadata(std::string id,
										  semantic_version version,
										  std::string label,
										  std::string category,
										  std::int32_t priority = 0) const;
	...
	class barrier_model
	{
	  public:
		...
		[[nodiscard]] barrier_model effect(barrier_effect value,
										   std::string output_label = {}) const;
		[[nodiscard]] barrier_model path(value_path value) const;
		[[nodiscard]] barrier_model metadata(std::string id,
											 semantic_version version,
											 std::string label,
											 std::string category,
											 std::int32_t priority = 0) const;
	...
	class taint_policy
	{
	  public:
		...
		[[nodiscard]] taint_policy propagate(std::string input_label,
											 std::string output_label) const;
	...
	[[nodiscard]] result<cfg_availability> inspect_cfg(const workspace& workspace,
													   analysis_scope scope = analysis_scope::all(),
													   execution_context execution = {});
	[[nodiscard]] result<taint_report>
	run_taint(const workspace& workspace, taint_policy policy, flow_options options = {});
	...
	[[nodiscard]] result<resource_report> check_resource_protocol(const workspace& workspace,
	...
	[[nodiscard]] result<effect_summary_report> build_effect_summaries(const workspace& workspace,
