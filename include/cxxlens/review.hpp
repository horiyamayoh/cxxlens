...
/** @file review.hpp @brief Stable review contract declarations. */
...
namespace cxxlens::review
{
	...
	{
	  public:
		[[nodiscard]] static result<diff_view> from_unified_diff(std::string_view input,
																 path repository_root = {},
																 diff_options options = {});
		[[nodiscard]] static result<diff_view> from_git(path repository,
														std::string base_ref,
														diff_options options = {},
														execution_context execution = {});
		[[nodiscard]] bool contains(const source_span& span,
									diff_side side = diff_side::after) const;
	...
	class workflow
	{
	  public:
		...
		[[nodiscard]] result<review_report> run(const workspace& workspace,
