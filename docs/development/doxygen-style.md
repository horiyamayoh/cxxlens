...
公開宣言の意味、安定したエラー、lifetime、threading、versioning は
`schemas/cxxlens_ng_public_api_catalog.yaml` と public header が所有する。Doxygen は宣言コメントの
構文と対応する API 説明を検査し、AST の固定件数や実装ファイルの内容を authority にしない。
