...
cmake --build --preset docs --target cxxlens-ng-release-contract-check
```

release/version compatibility request は次で side-effect-free に検査できます。request の `operation` は
command と一致させ、`doctor` では runtime qualification evidence を含めます。どちらも JSON の
`cxxlens.ng-compatibility-report.v1` を返し、暗黙 fallback は行いません。

```sh
python3 tools/quality/check_ng_release_contract.py inspect --request request.yaml
python3 tools/quality/check_ng_release_contract.py doctor --request doctor-request.yaml
```
