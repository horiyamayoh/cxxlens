...
> contract-driven, issue-tracked, evidence-closed
...
1. consumer / use case を確定する
2. semantics、invariants、non-goals を定義する
3. identity、value types、condition、interpretation、partiality を定義する
4. schema、registry、catalog、version evolution を定義する
5. implementation と独立した validator を実装する
6. positive / negative / property / fault test を実装する
7. public API と production implementation を実装する
8. provider → claim → store → query → recipe の vertical path を接続する
9. typed / dynamic、memory / SQLite、in-process / process parity を検証する
10. example、negative example、install consumer、Doxygen を追加する
11. support matrix と qualification evidence を更新する
12. experimental period と独立 consumer evidence を経て stable admission を判断する
