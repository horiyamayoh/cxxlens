...
現在の installed header は NG Foundation Migration の移行 surface です。次世代 public API の状態は
...
各 API family の exact signature、stable error、lifetime、threading、versioning、invariant、実装/正例/負例は
`schemas/cxxlens_ng_public_api_catalog.yaml` が authority です。利用例は `docs/tutorials/README.md` と
`examples/sdk/` にあります。
