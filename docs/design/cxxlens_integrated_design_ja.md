...
struct workspace_options {
  ...
  std::size_t compilation_database_byte_budget{256U * 1024U * 1024U};
  std::size_t compilation_database_entry_budget{1'000'000U};
  std::size_t compilation_database_string_byte_budget{64U * 1024U * 1024U};
...
2. JSON Compilation Database（通常は `compile_commands.json`）を load。read 前の byte budget と
   container entry / decoded string / nesting budget を適用し、超過は `core.budget-exhausted` に
   `reason`、`limit`、`observed` を付けて返す。既定 byte budget は 256 MiB で、caller が
   `workspace_options` から明示的に変更できる。
3. `arguments` を優先し、`command` しかない場合は互換 parser で argv 化するが shell 実行しない。
   parser は RFC 8259 の string escape、UTF-8、surrogate pair、number/boolean/null を受理し、未知 field
   は必須 field の解釈から分離して安全に skip する。不正 UTF-8、unpaired surrogate、malformed number、
   duplicate key は `workspace.compile-database-invalid` と stable reason/offset で拒否する。
...
`workspace_fixture::open` は value-owned source と compilation database を memory filesystem port
へ載せ、通常の workspace catalog、scheduler、Clang overlay VFS、extractor、reducer、fact store を通す。
host 側へ temporary source を作らず、fixture の追加順、jobs 数、process cwd/root が semantic identity や
canonical facts/coverage/doctor output を変えてはならない。
...
- `m1-acceptance`: exact Clang 22 の production path、M1 catalog/fact-kind traceability、
  multi-TU/multi-variant/macro、jobs/seed/order/root/repeat、memory/SQLite、cold/warm、failure/lifetime/install を
  一つの required status で検証する。
...
5. LLVM mapping
6. category 別の positive/negative/ambiguous semantic fixtures と expected outcome
...
`implementation_state: conformant` かつ `readiness.state: complete` の API は、
`cxxlens_api_test_coverage.yaml` に signature fingerprint で exactly once 登録し、`tests/unit/` から
公開関数仕様を直接検査する unit case と、通常 public header または installed consumer から文書化済み
use case/requirement の期待結果を検査する acceptance case をそれぞれ一件以上持たなければならない。
family API は declaration を `;` で分離した全 member を認証し、positive/negative/ambiguous はそれぞれ
case ID、CTest ID、evidence path、expected outcome を持つ。同一 implementation evidence 一覧の複製は
fixture 証跡ではない。M0/M1/M2 completion manifest の和集合は全 complete API 集合と完全一致し、重複も
欠落も許さない。派生 `cxxlens_api_test_coverage_report.json` の uncovered API/member/use-case は常に空とする。
...
M1 completion の規範 manifest は `cxxlens_m1_completion.yaml` とし、exact declaration を持つ全 M1 API、
supported fact kind、#13–#22 の invariant/test/fixture 所有を一対一に追跡する。宣言未確定 API は
deferred blocker として列挙し、実装済みと数えない。clean checkout の単一 gate は次である。

```sh
CXX=clang++-22 cmake --preset m1-acceptance
cmake --build --preset m1-acceptance --target cxxlens-m1-acceptance
```
...
[ ] invariant/schema
[ ] direct unit + public/installed use-case acceptance
[ ] positive/negative/ambiguous case ID + expected outcome
[ ] family member/use-case/requirement coverage report = zero gaps
...
### 39.18 rules・report package candidate

Issue #47 の規範契約は `docs/design/package_contract_47.md` と
`schemas/cxxlens_package_contract_candidates.yaml#issue-47` である。rule は `(normalized id,
semantic version)` で識別し、immutable builder、selector requirement union、shared query、canonical
finding reduction を用いる。suppression は finding ID 作成後に typed key と明示 precedence で適用し、
suppressed row と coverage/provenance を消去しない。

`report::render` は #43/#48/#49/#51 の authoritative value を再計算・再解釈しない pure projection
である。UTF-8/LF、canonical order/escaping、path redaction、finite output budget を固定し、truncation は
...
認証集合との完全一致と未被覆0件は `cxxlens-api-contract-check` が検査する。
以下の表は package group の非規範的な案内であり、全 124 entry の一致は
...
| API-TEST-007 | `workspace_fixture::open` | memory VFS production-path workspace fixture |
