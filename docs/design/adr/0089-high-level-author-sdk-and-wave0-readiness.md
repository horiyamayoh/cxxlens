...
従来の高水準 author target は名前に反して portable provider helper だけの target ではない。公開
...
`cxxlens::sdk` / `cxxlens` package は高水準 author SDK とする。公開 umbrella は relation、claim、store、query、provider、testing、recipe
