# ADR 0061: static/shared installed package は同一 matrix の直接試験で relocation を検証する
...
release workflow は static/shared matrix の全インストール試験に依存し、対応環境は通常の support matrix に宣言する。試験が一件でも失敗した場合、package job は実行しない。
