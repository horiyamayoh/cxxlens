...
`cxxlens.clang22-materialization-contract.v2` とする。
...
v2 request/response の version は exact `2.0.0` とし、missing/unknown field、unknown version、identity/digest mismatch を effect 前または
...
として解釈せず、v2 rollout が supersede する。v2 に implicit migration、adjacent version fallback、unknown-field preservation、caller-selected
default を設けない。request は exact project catalog census、expected parent publication、
...
trust_policy_digest = semantic_digest(
  ...
  tuple([policy ID, execution profile, provider ID/version/semantic contract,
         protocol major/minor, required qualification, worker sandbox digest,
...
condition は originating task の request-level universe と exact singleton condition fragment、scope は `project_id`、assumptions は report の sorted
assumption IDs に `"assumption-set:" + semantic_digest("cxxlens.clang22-assumption-set.v1", canonical tuple)` を適用した strong ID、verification modalities は canonical
sorted list とする。同じ semantic content に複数の provenance occurrence があれば losslessly report し、Store claim set の unique content count と
...
さらに Store の exact snapshot manifest と invocation publication record、reopened descriptor/partition/row/annotation/coverage/unresolved/cursor/export projection
...
report v2 schema は top-level `response_kind` を discriminator とする closed union である。両 branch は schema/version、failed/passed result、generated
...
`scope.clang22-installed-adoption-gap` / `#181` / `[DF-0182, DF-0187, DF-0191, DF-0192]` の `tracked-gap` である場合に限り、normal release evaluation が request/report/set の
...
元の installed boundary と observation v2 authority は Issue #182 の authority-only unit である。Issue #181 の preflight で見つかった DF-0191/DF-0192
に対する machine v2、SDK registry conformance、Store projection、release/checker amendment は Issue #181 の独立した authority-first checkpoint とする。
独立反証 review が exact binding を受理して両 DF を accepted / may-proceed にするまでは runtime/tool implementation を再開しない。受理後、runtime と
...
- Issue #181 は DF-0191/DF-0192 の independently accepted v2 machine/Store authority と SDK registry projectionを変更せず、tool、runtime/store
  integration と四件の release evidence を同一 contract に実装する必要がある。

## Verification

contract/checker は exact JSON version、bounded raw-input response、phase-authentic failure union、catalog census、12-descriptor engine admission、complete
...
result completeness、one expected-parent transaction、memory/reopened-SQLite parity、typed response/errors、static/shared matrix を positive/negative fixture
で検証する。
