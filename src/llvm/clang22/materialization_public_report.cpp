...
namespace cxxlens::detail::clang22::materialization
{
	namespace
	{
						  ...
						  const materialization_occurrence_receipt& receipt)
		{
			...
			if (receipt.occurrence_payload_digest != manifest.occurrence_payload_digest ||
				...
				return sdk::unexpected({"materialization.report-invalid",
										"installation",
										"occurrence-inventory-binding"});
			for (std::size_t index{}; index < manifest.files.size(); ++index)
				if (receipt.files[index].authority != manifest.files[index])
					return sdk::unexpected({"materialization.report-invalid",
		...
		const sdk::snapshot_manifest& candidate_manifest)
	{
		try
		{
			...
			std::optional<std::string> capacity_guarantee_digest;
			if (has_task_reports)
			{
				...
				capacity_guarantee_digest = *guarantee_digest->as_string();
			...
			if (!task_results || !capacity_guarantee_digest)
				return sdk::unexpected({"materialization.report-invalid",
										"report.capacity",
										"task-authority-required"});
			auto stages = claim_stages_json(
				task_results->results, *store, *capacity_guarantee_digest);
			if (!stages)
