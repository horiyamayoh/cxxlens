...
namespace cxxlens::detail::clang22::materialization
{
	...
	decode_materialization_v4_prior_artifact(const std::span<const std::byte> bytes,
											 const materialization_prior_artifact_limits& limits)
	{
		if (!valid_artifact_limits(limits) || bytes.empty() || bytes.size() > limits.max_bytes)
											  ...
											  const materialization_prior_artifact_limits& limits)
	{
		...
		return install_sidecar_spool(root, *path, **storage, limits);
	}
		...
		const materialization_prior_artifact_limits& limits)
	{
		...
		auto spool = spool_sidecar(*opened, limits);
		if (!spool)
