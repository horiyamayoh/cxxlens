...
namespace cxxlens::detail::clang22::materialization
{
	namespace
	{
		...
		encode_spooled_materialization_prior_artifact(
			...
			const materialization_prior_artifact_limits& limits)
		{
			if (!prior_artifact_limits_valid(limits) || tasks.empty() ||
	...
	decode_materialization_prior_artifact(const std::span<const std::byte> artifact_bytes,
										  const materialization_prior_artifact_limits& limits)
	{
		if (!prior_artifact_limits_valid(limits) || artifact_bytes.empty() ||
										...
										const materialization_prior_artifact_limits& limits)
	{
		...
		auto sidecar = spool_prior_artifact_sidecar(*opened, limits);
		if (!sidecar)
		...
		const materialization_prior_artifact_limits& limits)
	{
		...
		if (!path)
			return sdk::unexpected(std::move(path.error()));
		return install_prior_artifact_sidecar(root, *path, **encoded, limits);
	}
