...
namespace cxxlens::detail::clang22
{
	 ...
	 * task-local value; it never resolves a path from the ambient filesystem.  A validated
	 * snapshot is available only after every manifest-declared blob has been sealed.
	 */
	class source_closure_spool final : public source_closure_transfer_sink
	{
	  ...
	  private:
		...
		std::string current_blob_bytes_;
