...
namespace cxxlens::detail::clang22
{
	namespace
	{
		...
		[[nodiscard]] sdk::result<std::vector<UChar>> normalize_nfc(
			const std::vector<UChar>& utf16)
		{
			if (utf16.size() >
			...
			const auto written = unorm2_normalize(normalizer,
											 utf16.data(),
											 input_size,
											 normalized.data(),
											 normalized_size,
	...
	sdk::result<bool> is_nfc_utf8(const std::string_view value)
	{
		...
		if (U_FAILURE(status) || normalizer == nullptr)
			return sdk::unexpected(unicode_error("nfc-instance"));
		const UBool normalized = unorm2_isNormalized(normalizer,
													utf16->data(),
	...
	sdk::result<std::string> nfc_casefold_utf8(const std::string_view value)
	{
		...
		const auto input_size = static_cast<std::int32_t>(utf16->size());
		UErrorCode status = U_ZERO_ERROR;
		const auto folded_size = u_strFoldCase(
			nullptr, 0, utf16->data(), input_size, U_FOLD_CASE_DEFAULT, &status);
		if (status != U_BUFFER_OVERFLOW_ERROR && U_FAILURE(status))
			return sdk::unexpected(unicode_error("casefold-size"));
		status = U_ZERO_ERROR;
		std::vector<UChar> folded(static_cast<std::size_t>(folded_size));
		const auto written = u_strFoldCase(folded.data(),
										 folded_size,
										 utf16->data(),
										 input_size,
										 U_FOLD_CASE_DEFAULT,
