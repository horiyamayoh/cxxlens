...
/*
 * SQLite is intentionally loaded as a private runtime dependency in this target, so the build does
 * not include sqlite3.h.  These are the C ABI tags used by the SQLite VFS contract.  Keeping the
 * tags at global scope is important: every callback function pointer in every translation unit then
 * uses the same opaque C type, instead of separately named C++ namespace types that merely happen
...
namespace cxxlens::sdk::sqlite_vfs_abi
{
