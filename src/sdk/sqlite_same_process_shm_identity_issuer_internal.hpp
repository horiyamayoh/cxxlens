...
namespace cxxlens::sdk
{
	...
	/**
	 * Move-only presenter for one exact live registry/family control plus an asserted
	 * owner/request.
	 *
	 ...
	 * claim that crosses the allocation cut remains burned on a later stale/exhausted failure and
	...
	class sqlite_shm_issued_reader_effect_identity
	{
	  public:
		...
		sqlite_shm_issued_reader_effect_identity(const sqlite_shm_issued_reader_effect_identity&) =
			delete;
		sqlite_shm_issued_reader_effect_identity&
	...
	class sqlite_shm_reader_zero_effect_identity_validation_capability final
	{
	  public:
		...
		sqlite_shm_reader_zero_effect_identity_validation_capability&
		operator=(sqlite_shm_reader_zero_effect_identity_validation_capability&&) = delete;
		sqlite_shm_reader_zero_effect_identity_validation_capability(
			const sqlite_shm_reader_zero_effect_identity_validation_capability&) = delete;
		sqlite_shm_reader_zero_effect_identity_validation_capability&
		operator=(const sqlite_shm_reader_zero_effect_identity_validation_capability&) = delete;
	...
	class sqlite_shm_reader_mapped_effect_identity_validation_capability final
	{
	  public:
		...
		sqlite_shm_reader_mapped_effect_identity_validation_capability&
		operator=(sqlite_shm_reader_mapped_effect_identity_validation_capability&&) = delete;
		sqlite_shm_reader_mapped_effect_identity_validation_capability(
			const sqlite_shm_reader_mapped_effect_identity_validation_capability&) = delete;
		sqlite_shm_reader_mapped_effect_identity_validation_capability&
		operator=(const sqlite_shm_reader_mapped_effect_identity_validation_capability&) = delete;
	 ...
	 * process registry mutex. A future caller therefore cannot carry this bridge's lock across
	 * native execution. Public aggregate equality is never validation: every operation checks the
	 * private control provenance, process epoch, registry owner-equivalence, scope, domain, and
	 * role. The issuer retains no ever-issued ledger; nonreuse comes from its single checked
	 * no-wrap sequence. U2a1a mints and validates callback-invocation, native/zero-effect, and
	 * session-terminal identity domains from this one process source. Full session-transaction
	 * terminal validation, including the exact session owner and no-live-lock condition, remains
	 * deferred; this facade provides the identity primitive rather than that semantic validator.
	 */
	class sqlite_shm_process_global_identity_issuer
	{
	  public:
		...
		[[nodiscard]] sqlite_shm_lease_result<void>
		retire_session_terminal(const sqlite_shm_reader_lifecycle_identity_scope& scope,
