...
/** @file qa.hpp @brief Issue #51 non-installed QA Contract Candidate. */
...
namespace cxxlens::qa
{
	...
	class profile
	{
	  public:
		...
		[[nodiscard]] profile build_target(std::string value, step_requirement requirement = step_requirement::required) const;
		[[nodiscard]] profile test_argv(std::vector<std::string> value, step_requirement requirement = step_requirement::required) const;
	...
	[[nodiscard]] result<coverage_data> import_source_coverage(
		const workspace& workspace, coverage_import_options options, execution_context execution = {});
} // namespace cxxlens::qa
