...
/**
 * @file graph.hpp
 * @brief Issue #46 Contract Candidate declarations for semantic graph APIs.
 *
 * This fragment is not installed. Issue #53 owns stable-header integration and Issue #54 owns
 * the frozen transition. Issue #46 does not provide production implementations.
 */
...
namespace cxxlens::graph
{
	...
	class semantic_graph
	{
	  public:
		...
		[[nodiscard]] result<semantic_graph> subgraph(
			std::vector<graph_node_id> roots, graph_options options = {}) const;
	...
	[[nodiscard]] result<semantic_graph> class_hierarchy(
		const workspace& workspace, graph_options options = {});
	[[nodiscard]] result<semantic_graph> override_graph(
		const workspace& workspace, graph_options options = {});
	[[nodiscard]] result<semantic_graph> call_graph(
		const workspace& workspace, graph_options options = {});
	[[nodiscard]] result<semantic_graph> include_graph(
		const workspace& workspace, graph_options options = {});
	...
	class impact_query
	{
	  public:
		...
		[[nodiscard]] result<semantic_graph> run(
