...
/** @file models.hpp @brief Issue #50 non-installed API model Contract Candidate. */
...
namespace cxxlens::models
{
	...
	class api_model_pack
	{
	  public:
		...
		[[nodiscard]] result<api_model_pack> function(std::string qualified_name, api_effects effects) const;
		[[nodiscard]] result<api_model_pack> method(
			std::string receiver_type, std::string method_name, api_effects effects) const;
		[[nodiscard]] result<api_model_pack> replacement(std::string old_api, std::string new_api) const;
		[[nodiscard]] result<api_model_pack> source(std::string name, flow::source_model model) const;
		[[nodiscard]] result<api_model_pack> sink(std::string name, flow::sink_model model) const;
		[[nodiscard]] result<api_model_pack> barrier(std::string name, flow::barrier_model model) const;
		[[nodiscard]] result<api_model_pack> merge(
			const api_model_pack& other, model_precedence precedence = model_precedence::reject_conflicts) const;
		[[nodiscard]] static result<api_model_pack> load(path yaml_or_json, model_pack_options options = {});
