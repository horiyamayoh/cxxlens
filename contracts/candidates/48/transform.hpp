...
/** @file transform.hpp @brief Issue #48 non-installed transform Contract Candidate. */
...
namespace cxxlens::transform
{
	...
	class edit_plan
	{
	  public:
		...
		[[nodiscard]] std::span<const unresolved> unresolved_items() const noexcept;
		[[nodiscard]] const coverage_report& coverage() const noexcept;
		[[nodiscard]] result<preview_result> preview_unified_diff(preview_options options = {}) const;
		[[nodiscard]] std::string to_json() const;
		[[nodiscard]] result<apply_result> apply(workspace& workspace, apply_options options = {}) const;
	...
	class replacement_template
	{
	  public:
		...
		[[nodiscard]] replacement_template argument(std::size_t index, std::string placeholder) const;
	...
	[[nodiscard]] codemod replace_function_call(std::string old_qualified, std::string new_qualified);
	[[nodiscard]] codemod replace_method_call(
		std::string receiver_type, std::string old_method, std::string new_method);
	[[nodiscard]] codemod rewrite_calls(select::call_selector selector, replacement_template replacement);
