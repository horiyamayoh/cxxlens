...
/** @file rules_report.hpp @brief Issue #47 non-installed Contract Candidate declarations. */
...
namespace cxxlens::rules
{
	...
	class suppression_policy
	{
	  public:
		...
		[[nodiscard]] suppression_policy category(std::string value, std::string justification) const;
	...
	class rule
	{
	  public:
		...
		[[nodiscard]] result<finding_set> run(const workspace& workspace, rule_run_options options = {}) const;
		[[nodiscard]] std::string to_json() const;
...
namespace cxxlens::report
{
	...
	[[nodiscard]] result<rendered_report> render(const transform::edit_plan& value, options settings = {});
	[[nodiscard]] result<rendered_report> render(const generate::generation_plan& value, options settings = {});
	[[nodiscard]] result<rendered_report> render(const review::review_report& value, options settings = {});
} // namespace cxxlens::report
