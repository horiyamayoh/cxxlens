...
auto main() -> int
{
	...
	bool passed = check(registry.all().size() == 24U, "public schema registry is incomplete");
	...
	passed &= check(registry.find("cxxlens.scheduler-trace.v1", {1U, 0U, 0U, {}}).has_value(),
					"scheduler trace schema is not registered");
	passed &= check(registry.find("cxxlens.provisioning-trace.v1", {1U, 0U, 0U, {}}).has_value(),
