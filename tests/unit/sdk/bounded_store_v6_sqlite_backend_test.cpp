...
namespace
{
	...
	[[nodiscard]] std::vector<std::byte> tuple(const std::string_view value)
	{
		auto encoded = canonical_binary(canonical_value::from_tuple(
			{canonical_value::from_string(std::string{value})}));
	...
	class report_writer final : public detail::bounded_store_report_tail_writer
	{
	  public:
		...
		reserve_maximum_tail(const std::uint64_t tail, const std::uint64_t maximum) override
		{
			...
			reserved_ = true;
			return detail::bounded_store_v6_report_tail_reservation{
				"writer:sqlite", "spool:sqlite", "reservation:sqlite", 1024U, tail, maximum, maximum};
		}
	...
	void sqlite_v6_phase_round_trip()
	{
		...
		detail::bounded_store_v6_session_metadata metadata{
			detail::bounded_store_v6_backend::sqlite,
		...
		detail::bounded_store_v6_external_census census{
			1U, 1U, 2U, 0U, 0U, 0U, 0U, 0U,
			static_cast<std::uint64_t>(bytes.size()), binary, "authority:sqlite-test"};
		...
		detail::bounded_store_v6_task_receipt receipt{
			"task:sqlite", 0U, 1U, 2U, 0U, 0U, 0U, 0U, 0U,
			static_cast<std::uint64_t>(bytes.size()), binary, "receipt:sqlite"};
		...
		auto task = detail::bounded_store_v6_phase_core::seal_task_source(
			std::move(receipt), std::move(*task_source));
		require(task.has_value(), "sqlite task seal failed");
	require(static_cast<bool>(detail::bounded_store_v6_phase_core::stage_from_source(*prepared,
																									 std::move(*task))),
				"sqlite staging failed");
	require(static_cast<bool>(detail::bounded_store_v6_phase_core::seal_prepared_publication(*prepared)),
				"sqlite staging seal failed");
		auto expected_source = detail::make_bounded_store_v6_memory_expected_semantic_source(source);
	...
	require(terminal.has_value() && terminal->observation().terminal ==
