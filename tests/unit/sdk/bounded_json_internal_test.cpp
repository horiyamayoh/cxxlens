...
namespace
{
				...
				const json_limits& limits = small_limits())
	{
		...
		require(parsed.error().code == "sdk.json-invalid",
	...
	void caller_policy_and_error_projection()
	{
		auto limits = small_limits();
		const json_parse_contract value_contract{
			.error_code = "test.capture-json-invalid",
		...
		auto array = parse_json_value(R"([{"file":"src/main.cpp"}])", limits, value_contract);
		require(array && array->as_array() != nullptr,
				"caller-selected top-level array was rejected");

		auto exact_decimal = parse_json_value("1e2", limits, value_contract);
		require(!exact_decimal && exact_decimal.error().code == "test.capture-json-invalid" &&
					exact_decimal.error().field == "compile_commands" &&
					exact_decimal.error().detail == "integer-expected",
				"integer-token policy or error projection drifted");

		auto malformed = parse_json_value(R"({"x":)", limits, value_contract);
